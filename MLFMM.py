...
FH-DES/deslib/util/MLFMM.py /
@redavtalab
redavtalab Adding MLFMM
Latest commit ffc0421 1 minute ago
 History
 1 contributor
444 lines (337 sloc)  14.3 KB
...
class MLFMM:
    def __init__(self, theta=0.5, gamma=1, mu=4, num_layer=3, random_state=0):
        self.theta = theta
        self.gamma = gamma
        self.mu = mu
    ...
    def membership(self,x_q):
        ...
        hboxC = np.add(self.V, self.W) / 2
        boxes_W = self.W.reshape(NO_hypeboxes, 1, n_features)
        boxes_V = self.V.reshape(NO_hypeboxes, 1, n_features)
        boxes_center = hboxC.reshape(NO_hypeboxes, 1, n_features)
        halfsize = ((boxes_W - boxes_V) / 2).reshape(NO_hypeboxes, 1, n_features)
        d = np.abs(boxes_center - x_q) - halfsize
        d[d < 0] = 0
...
class Animator:
    ...
    '''
    def __init__(self, box_history, train_patterns, classes, frame_rate, theta, gamma,
        ...
        self.ax.set_title('θ = {} and γ = {}'.format(theta, gamma))
        self.rectangles = []
        self.scatters = []
...
class MLFMM:
    def __init__(self, theta=0.5, gamma=1, num_level = 3, cur_level = 0, animate = False,  random_state=None):
        """
        Initialize the Multi-Level Fuzzy Min-Max (MLFMM) classification algorithm.

        Parameters
        ----------
        theta : float, optional (default=0.5)
            The maximum size of hyperboxes.
        gamma: float, optional (default=1.0)
            The fuzzy sensitivity parameter of membership function
        num_levels : int, optional (default=3)
            The number of levels for the hierarchy.
        cur_level : current level id

        random_state : int or None, optional (default=None)
            The random seed to use for generating random numbers. If None, use the default random seed.
        """
    ...
    def membership(self, pattern):
        ...
        a = np.maximum(0, (1 - np.maximum(0, (self.gamma * np.minimum(1, pattern - max_pts)))))
        b = np.maximum(0, (1 - np.maximum(0, (self.gamma * np.minimum(1, min_pts - pattern)))))
    ...
    def train_pattern(self, X, Y):
        ...
        if target not in self.classes:
            ...
            while True:
                ...
                if self.theta * len(np.unique(self.classes)) >= np.sum(max_new - min_new):
...
Blog
About
FH-DES/MLFMM.py at main · redavtalab/FH-DES
